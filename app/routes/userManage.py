...
from app.utils.mongo2dict import m2d_fields
        ...
        if userInfo is None:
            ...
            if userInfo.check_password(password):
                ...
                return jsonReturn.trueReturn({'id': str(userInfo.id), 'username': userInfo.username,
                                              'referrer': userInfo.referrer.username, 'vip': userInfo.vip,
...
# 用户管理
# 查找用户列表
@bp.route('/admin/user_list', methods=['GET'])
def user_list():
    """
    params: {
        page:  当前第几页，默认 1
    ...
    query_set = User.objects().filter(username__ne='admin')
    if req_args:
        ...
        if keyword:
            query_set = query_set.filter(username__contains=keyword)
        if sort:
            if sort == "升序":
                query_set = query_set.order_by('+username')
            else:
                query_set = query_set.order_by('-username')
        else:
            # 时间逆序，最近的时间越大
...
def add_user():
    ...
    username = req_json.get('userName')
    password = req_json.get('userPwd')
    user_type = req_json.get('userType')
    if not username or not password:
        return jsonReturn.falseReturn('', '用户名和密码不能为空')
    if User.objects(username=username).first():
        return jsonReturn.falseReturn('', '用户名已存在')
    referrer = User.objects(username='admin').first()
    user = User.register(username, password, referrer, user_type)
    # 这里User的password做了特别处理，需要加密保存，单独写了方法新建用户
    # password 是通过@property 和 @property.setter 来定义的，真正的 password 是 _password
    print(user.to_mongo())
    if user.id:
...
def del_user():
    ...
    if not user_id:
        return jsonReturn.falseReturn(request.path, '请上传必要的参数 user_id')
    try:
...
# 用户升级为代理
@bp.route('/admin/upgrade_as_agent', methods=['POST'])
def upgrade_as_agent():
    """
    获取用户信息
    :return: json
    """
    ...
    if user:
        user.save_as_agent()
        return jsonReturn.trueReturn(user, "成功")
    return jsonReturn.falseReturn('', '用户注册失败')
